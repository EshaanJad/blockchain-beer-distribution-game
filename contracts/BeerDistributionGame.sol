...
contract BeerDistributionGame is Ownable {
    // Game parameters
    uint256 public constant GAME_LENGTH = 70; // Total number of weeks (increased from 52 to 70)
    uint256 public initialInventory = 12; // Changed from 0 to 8
    ...
    function shipProduct(Role from, Role to, uint256 amount) private {
        // No-op if nothing to ship
        if (amount == 0) {
            return;
        }
        // Zero-delay shipping: deliver immediately
        if (shippingDelayPeriod == 0) {
            SupplyChainMember storage receiver = supplyChainMembers[to];
            uint256 remaining = amount;
            // 1) Clear backlog first
            if (receiver.backorderedAmount > 0) {
                uint256 toFulfill = receiver.backorderedAmount <= remaining
                    ? receiver.backorderedAmount
                    : remaining;
                receiver.backorderedAmount -= toFulfill;
                remaining -= toFulfill;
                // Track outgoing shipment on sender
                if (from != Role.RETAILER && toFulfill > 0) {
                    supplyChainMembers[from].outgoingShipment += toFulfill;
                }
            }
            // 2) Add any remaining shipment to inventory
            if (remaining > 0) {
                receiver.currentInventory += remaining;
            }
            // 3) Update incomingShipment for display
            receiver.incomingShipment += amount;
            emit ShipmentMade(from, to, amount, currentWeek);
            return;
        }
        // Delayed shipping: enqueue in pipeline
