/**
 * Forecasted Crocs Supply Chain Simulation Script with Two Week Order Delay and One Week Shipping Delay
 * 
 ...
 * 
 * Parameters:
 * - Order Delay: 2 weeks (orders take 2 weeks to be received by the upstream supplier)
...
async function runSimulation(customerDemandData, periods, useBlockchain, label) {
    ...
    // Set 2 week order delay
    await game.setOrderDelayPeriod(2);
    console.log("Order delay set to 2 weeks");
    ...
    const delayPath = path.join(delayDir, `data_${visibilityLabel}_forecasted_crocs_two_one_delay_${periods}_periods.json`);
    
    // 2. New location (forecasted directory)
    const forecastedDir = path.join(__dirname, '../visualization/data/simulations/forecasted');
    const forecastedPath = path.join(forecastedDir, `data_${visibilityLabel}_forecasted_crocs_two_one_delay_${periods}_periods.json`);
...
async function main() {
    try {
        console.log("Starting Forecasted Crocs Supply Chain Simulations with Two Week Order Delay and One Week Shipping Delay");
        console.log("================================================================================================\n");
        ...
        // Create summary file
        const summaryPath = path.join(__dirname, '../visualization/forecasted_two_one_delay_simulation_summary.csv');
        ...
        const traditionalResult = await runSimulation(
            ...
            `traditional_forecasted_two_one_delay_${maxPeriods}`
        );
        ...
        const blockchainResult = await runSimulation(
            ...
            `blockchain_forecasted_two_one_delay_${maxPeriods}`
        );
