/**
 * Forecasted Crocs Supply Chain Simulation Script with Zero Order Delay and Two Week Shipping Delay
 * 
...
async function runSimulation(customerDemandData, periods, useBlockchain, label) {
    ...
    const delayPath = path.join(delayDir, `data_${visibilityLabel}_forecasted_crocs_two_week_delay_${periods}_periods.json`);
    
    // 2. New location (forecasted directory)
    const forecastedDir = path.join(__dirname, '../visualization/data/simulations/forecasted');
    const forecastedPath = path.join(forecastedDir, `data_${visibilityLabel}_forecasted_crocs_two_week_delay_${periods}_periods.json`);
...
async function main() {
    try {
        console.log("Starting Forecasted Crocs Supply Chain Simulations with Zero Order Delay and Two Week Shipping Delay");
        console.log("================================================================================================\n");
        ...
        // Create summary file
        const summaryPath = path.join(__dirname, '../visualization/forecasted_two_week_delay_simulation_summary.csv');
        ...
        const traditionalResult = await runSimulation(
            ...
            `traditional_forecasted_two_week_delay_${maxPeriods}`
        );
        ...
        const blockchainResult = await runSimulation(
            ...
            `blockchain_forecasted_two_week_delay_${maxPeriods}`
        );
