 ...
 * - In the blockchain scenario, the distributor is the most downstream blockchain participant
 * - Retailer and wholesaler don't participate in the blockchain network
 * 
 * Parameters:
 * - Order Delay: 1 week
...
async function getStermanOrderData(gameContract, role, roleHasBlockchainVisibility, simulationIsBlockchainEnabled) {
    ...
    let lastReceivedOrder = 4;
    if (role === Role.RETAILER) {
        try {
            lastReceivedOrder = toNumber(await gameContract.getCurrentCustomerDemand());
        } catch (error) {
        ...
        try {
            if (currentWeek > 0) {
                lastReceivedOrder = toNumber(await gameContract.getMemberOrderForWeek(role - 1, currentWeek - 1));
            }
        } catch (error) {
    ...
    if (roleHasBlockchainVisibility) {
        // This role (Distributor or Factory) uses the blockchain policy.
        // Their L_hat signal is the Wholesaler's order to the Distributor.
        try {
            const wholesalerOrderToDistributor = await gameContract.getMemberOrderForWeek(Role.WHOLESALER, currentWeek);
            currentEndCustomerDemandForLhat = toNumber(wholesalerOrderToDistributor);
            console.log(`Role ${role} (${roleToString(role)}) (BC policy): Using Wholesaler's order to Distributor (${currentEndCustomerDemandForLhat}) as currentEndCustomerDemandForLhat`);
            ...
            // currentEndCustomerDemandForLhat will use its default (4) if wholesalerOrderToDistributor fetch fails.
        ...
        // This is Retailer or Wholesaler (using traditional policy) in a BC-enabled simulation,
    ...
    return {
        onHand, backlog, onOrder, lastReceivedOrder, previous_L_hat, role,
 ...
 * @param {boolean} roleIsActuallyVisible - Whether this specific role uses the blockchain policy
 * @param {boolean} overallSimModeIsBC - True if the overall simulation is the blockchain-enabled variant
 */
async function placeStermanOrder(gameContract, signer, role, roleIsActuallyVisible, overallSimModeIsBC) {
    console.log(`===== Placing Sterman order for ${roleToString(role)} =====`);
    
    // roleIsActuallyVisible determines if this specific role should use its blockchain-enhanced policy.
    // overallSimModeIsBC indicates the overall simulation mode, used for logging in getStermanOrderData.
    const orderData = await getStermanOrderData(gameContract, role, roleIsActuallyVisible, overallSimModeIsBC);
    ...
    // Only track gas costs for blockchain-enabled roles (distributor and factory)
    if (orderData.hasBlockchainVisibility) {
        try {
 ...
 * @param {boolean} factoryIsActuallyVisible - Whether the factory uses the blockchain policy
 * @param {boolean} overallSimModeIsBC - True if the overall simulation is the blockchain-enabled variant
 */
async function scheduleStermanProduction(gameContract, signer, factoryIsActuallyVisible, overallSimModeIsBC) {
    console.log(`===== Scheduling Sterman production for Factory =====`);
    // factoryIsActuallyVisible determines if the factory uses its blockchain-enhanced policy.
    // overallSimModeIsBC indicates the overall simulation mode, used for logging in getStermanOrderData.
    const orderData = await getStermanOrderData(gameContract, Role.FACTORY, factoryIsActuallyVisible, overallSimModeIsBC);
...
async function runGameWithStermanPolicy(gameContract, signers, simulationIsBlockchainEnabled, demandPattern = 0, weeks = 69) {
    console.log(`\n======= Starting Sterman ${simulationIsBlockchainEnabled ? 'Blockchain-Enabled (Hybrid: Retailer & Wholesaler Excluded)' : 'Traditional'} Simulation for ${weeks} weeks =======\n`);
    
    // In traditional mode, don't track any gas costs
    console.log(`Gas tracking: ${simulationIsBlockchainEnabled ? 'Enabled (only for Distributor & Factory)' : 'Disabled (no gas costs in traditional mode)'}`);
    if (!simulationIsBlockchainEnabled) {
        console.log('No transaction costs will be added to player costs in traditional mode');
    } else {
        console.log('Note: Retailer & Wholesaler use traditional policy. Distributor & Factory use blockchain policy with Wholesaler\'s orders as demand signal.');
    }
    ...
    for (let week = 0; week < weeks; week++) {
        ...
        try {
            ...
            const res = await placeStermanOrder(gameContract, signers[0], Role.RETAILER, false, simulationIsBlockchainEnabled);
        ...
        try {
            console.log("Wholesaler (Role 1) will use Traditional Sterman Policy.");
            const res = await placeStermanOrder(gameContract, signers[1], Role.WHOLESALER, false, simulationIsBlockchainEnabled);
        ...
        // 3. Distributor places order to factory - use blockchain visibility if enabled
        ...
        try {
            ...
            const res = await placeStermanOrder(gameContract, signers[2], Role.DISTRIBUTOR, simulationIsBlockchainEnabled, simulationIsBlockchainEnabled);
        ...
        try {
            ...
            const res = await scheduleStermanProduction(gameContract, signers[3], simulationIsBlockchainEnabled, simulationIsBlockchainEnabled);
...
async function main() {
    try {
        ...
        const simDir = path.join(vizDir, 'data/simulations/sterman/hybrid-retailer-wholesaler');
        ...
        const summaryObject = {
            DataSource: "hybrid_retailer_wholesaler_simulation",
