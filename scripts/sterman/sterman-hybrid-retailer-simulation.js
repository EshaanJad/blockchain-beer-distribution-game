 ...
 * - Other roles (Wholesaler, Distributor, Factory): Use Sterman's heuristic policy
 * 
 * It demonstrates how a hybrid approach with historical data for the Retailer
 * and Sterman's heuristic for other roles performs.
 * 
 * Parameters:
 * - Order Delay: 1 week
...
async function placeOrder(gameContract, signer, role, hasBlockchainVisibility) {
    ...
    // Retailer uses historical order data
    if (role === Role.RETAILER) {
        ...
        // All other roles use Sterman's heuristic
...
/**
 * Run the game with hybrid policy (Retailer uses historical data, others use Sterman)
 * 
 * @param {Contract} gameContract - The deployed BeerDistributionGame contract
...
async function runGameWithHybridPolicy(gameContract, signers, hasBlockchainVisibility, demandPattern = 0, weeks = 52) {
    console.log(`\n======= Starting Hybrid Retailer ${hasBlockchainVisibility ? 'Blockchain-Enabled' : 'Traditional'} Simulation for ${weeks} weeks =======\n`);
    ...
    for (let week = 0; week < weeks; week++) {
        ...
        // 2. Wholesaler places order to distributor (using Sterman)
...
async function main() {
    try {
        ...
        const traditionalPath = path.join(vizDir, 'data_traditional_hybrid_retailer.json');
        ...
        const blockchainPath = path.join(vizDir, 'data_blockchain_hybrid_retailer.json');
        ...
        // Save summary to CSV
        const summaryPath = path.join(vizDir, 'hybrid_retailer_simulation_summary.csv');
