/**
 * Extended Hybrid Policy Script for Beer Distribution Game
 * 
 ...
 * - Wholesaler orders
 * 
 * The upstream players (Distributor, Factory) use the algorithmic policy.
 * 
...
/**
 * Read extended order flow data from CSV file
    ...
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(filePath)
            ...
            .on('data', (data) => {
                ...
                const wholesalerOrder = parseFloat(data['Crocs → YY Orders ($)']); // Correct column name
            ...
            .on('end', () => {
                if (results.length === 0) {
                    ...
                    for (let i = 0; i < Math.min(3, results.length); i++) {
                        console.log(`  Period ${i+1}: Customer Demand = ${results[i].customerDemand}, Retailer Order = ${results[i].retailerOrder}, Wholesaler Order = ${results[i].wholesalerOrder}`);
                    }
                    resolve(results);
                }
            })
...
// Import the rest of the functions from runHybridRetailerPolicy.js
// getOrderData, placeAlgorithmicOrder, etc.
 ...
 * @param {number} role - The role placing the order (0 for retailer, 1 for wholesaler)
...
async function placeFixedOrder(gameContract, signer, orderAmount, role) {
    try {
        ...
        // Log for debugging
        const roleName = (role === Role.RETAILER) ? "Retailer" : "Wholesaler";
 ...
 * - Fixed order data from CSV for customer demand, retailer orders, and wholesaler orders
 * - Algorithmic policy for distributor and factory
 * 
 ...
 */
async function runGameWithExtendedHybridPolicy(gameContract, signers, orderFlowData, hasBlockchainVisibility = false, demandPattern = 3, weeks = 20) {
    try {
        ...
        console.error("Error in runGameWithExtendedHybridPolicy:", error);
        throw error;
    }
}
...
async function runSimulation(orderFlowData, periods, useBlockchain, label) {
    ...
    // Run simulation with extended hybrid policy
    const result = await runGameWithExtendedHybridPolicy(
        game,
    ...
    const dataPath = path.join(__dirname, `../visualization/data_${visibilityLabel}_extended_hybrid_${periods}_periods_full_${label}.json`);
...
async function main() {
    try {
        console.log("Starting Extended Hybrid Policy Simulations");
        console.log("=================================================\n");
...
module.exports = {
    readOrderFlowData,
    placeFixedOrder,
    runGameWithExtendedHybridPolicy,
    Role
}; 