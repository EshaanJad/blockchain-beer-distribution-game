/**
 * Policy Comparison Script for Beer Distribution Game - CONSTANT Demand
 * 
 * This script compares the performance of the same ordering policy with and without
 * blockchain visibility using constant demand to demonstrate the value of full supply chain visibility.
 */
...
async function main() {
    console.log("Starting Beer Distribution Game Policy Comparison - CONSTANT Demand");
    console.log("==========================================================\n");
    ...
    console.log("Running traditional game (without blockchain visibility, using CONSTANT demand)...");
    const traditionalGame = await BeerDistributionGame.deploy();

    // Run simulation without blockchain visibility - Pass DemandPattern.CONSTANT
    const traditionalResult = await runGameWithPolicy(
        traditionalGame,
        [retailer, wholesaler, distributor, factory],
        false, // No blockchain visibility
        DemandPattern.CONSTANT, // Specify CONSTANT demand
        52 // 52 weeks
    );
    ...
    const traditionalDataPath = path.join(__dirname, '../visualization/data_traditional_constant.json');
    try {
    ...
    console.log("\n\nRunning blockchain-enabled game (with full visibility, using CONSTANT demand)...");
    const blockchainGame = await BeerDistributionGame.deploy();

    // Run simulation with blockchain visibility - Pass DemandPattern.CONSTANT
    ...
    const blockchainDataPath = path.join(__dirname, '../visualization/data_blockchain_constant.json');
    try {
    ...
    // Compare results
    console.log("\n\nResults Comparison - CONSTANT Demand");
    console.log("====================================");
